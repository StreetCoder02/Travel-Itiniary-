...
Budget: INR ${budget} total for ${travelers} traveler${travelers > 1 ? 's' : ''}
...
Make sure the total cost fits within the INR ${budget} budget. Focus on authentic local experiences and practical information. Format the response as a structured JSON object.`;
}
...
function createStructuredItinerary(apiResponse: string, request: ItineraryRequest): ItineraryResponse {
  ...
  return {
    city,
    summary: `A personalized ${days}-day journey through ${city} featuring ${interests.join(', ')} experiences. Your ${accommodation} accommodation and ${transportation} transportation preferences have been considered to create the perfect adventure within your INR ${budget} budget.`,
    totalBudget: budget,
